...
// ── 軸馬の信頼度ラベル(T-3) — jiku_gap3(本命-3位の軸馬度差)を JRA/NAR別に3段階化 ──
// リーク無実データ(2026 day-of・n=1140)の場別3分位で較正(数値=本命の実複勝率):
//   JRA  断トツ≥21:61.5% / 優位12-21:48.7% / 拮抗<12:37.7%
//   NAR  断トツ≥17:70.8% / 優位12-17:66.8% / 拮抗<12:63.0%
// 絶対%表示は較正不十分で看板倒れになるため出さず、相対3段階のみ(表示専用・買い目非汚染)。
type JikuTier = "断トツ" | "優位" | "拮抗";
function jikuConfTier(gap3: number, isJra: boolean): JikuTier {
...
// JRA10場(中央)の会場名。これ以外はNAR(地方)とみなし、軸馬の信頼度を場別判定する。
const JRA_VENUE_NAMES = new Set([
  "札幌", "函館", "福島", "新潟", "東京", "中山", "中京", "京都", "阪神", "小倉",
]);
function isJraVenue(venue: string | undefined): boolean {
  return JRA_VENUE_NAMES.has(venue || "");
}

// 軸馬の信頼度バッジ（断トツ=緑 / 優位=青 / 拮抗=琥珀）
function JikuConfBadge({ tier }: { tier: JikuTier }) {
  const cls =
    tier === "断トツ"
  ...
  return (
    <span
      className={`text-[10px] font-bold px-1.5 py-0.5 rounded whitespace-nowrap ${cls}`}
