...
- **主要ファイル**:
  - `data/masters/kaisai_calendar.json`（259KB / 1,583 開催日 / git 管理対象）
  - `scripts/build_kaisai_calendar.py`（カレンダー構築スクリプト）
  - `src/scraper/kaisai_calendar_util.py`（`validate_race_against_calendar` 関数）
  - `frontend/src/pages/CalendarPage.tsx`（月別グリッド UI）
  - `src/dashboard.py`（`/api/kaisai_calendar` エンドポイント）
- **データソース**: `race.netkeiba.com` / `nar.netkeiba.com`（各 `calendar.html`）
- **カバレッジ**: 2022-01-01〜2026-12-31 / 1,583 開催日
- **UI**: 月別グリッド表示。日付クリックで成績ページまたは予想ページへ遷移（CalendarPage.tsx → `/results?date=YYYY-MM-DD` or `/today?date=YYYY-MM-DD`）
- **パイプライン hook**: `run_analysis_date.py` / `bulk_backfill_predictions.py` / `backfill_ml_from_cache.py` にカレンダーマスタとの照合を追加。非開催日を early exit で弾くことで不要スクレイピングを防止
- **設計思想**: race_id 構造解析（内部推論型）は脆弱。外部公式 = ground truth の原則を徹底。「実際にレースがあった日」のみをパイプラインが処理する責務を持つ
- **制約**: カレンダー更新は年 1 回（または重賞変更時）手動で `build_kaisai_calendar.py` を再実行する

### F-010 馬指数絶対/相対切替（Plan-γ Phase 5）
- **責務**: 馬カード能力軸の表示を「絶対偏差値（全馬比較）」と「相対偏差値（当該レース内比較）」でトグル切替
- **主要ファイル**:
  - `frontend/src/pages/TodayPage/HorseCardPC.tsx`（PC 馬カード）
  - `frontend/src/pages/TodayPage/HorseCardMobile.tsx`（モバイル馬カード）
  - `frontend/src/lib/featureFlags.ts`（`USE_RELATIVE_DEV` フラグ）
- **データソース**: pred.json の `race_relative_dev`（レース内 z-score、Plan-γ Phase 2 で追加）
- **計算式**: `50 + 10 × clip((ability_total − μ) / max(σ, 5.0), -3, 3)` → 範囲 20.0〜80.0
- **永続化**: LocalStorage に切替状態を保存し、ページリロード後も維持
- **切替条件**: `race_relative_dev` フィールドが NULL でないレース（field_count ≥ 5）のみで有効
...
- **更新タイミング**: レース直前オッズ（発走 15 分前から自動 fetch）。`_auto_fetch_post_races` でクールダウン付き更新
- **穴馬ハイライト条件**: `mark == "◎"` かつ `popularity >= 5`（人気薄の本命）→ オッズを金色テキストで強調
- **制約**: 帯広ばんえいはオッズ API 未対応（`tansho_odds = null`）

### F-012 horses マスター（D Phase 1+2+3）
- **責務**: race_log から全競走馬を集約した horses マスターテーブルを構築し、netkeiba_id で外部 ID 紐付け・同名異形式の統合を実現
- **主要ファイル**:
  - `data/masters/horses_master.db`（horses テーブル / git 管理対象）
  - `scripts/build_horses_master.py`（マスター構築・月次更新スクリプト）
  - `scripts/backfill_horses_netkeiba_id.py`（netkeiba_id 補完スクリプト）
  - `scripts/merge_duplicate_horses.py`（同名重複統合スクリプト）
  - `scripts/audit_pred_venue.py`（venue 整合性監査スクリプト）
- **規模**: 42,515 頭の `netkeiba_id` 補完済み（残 2,847 件は追加バックフィル継続中）
- **テーブル定義**:
  ```sql
  CREATE TABLE horses (
    horse_id    TEXT PRIMARY KEY,  -- 'nar_XXXXXXXX' or 'B_XXXXXXXX' 形式
    horse_name  TEXT,
    netkeiba_id TEXT,              -- netkeiba DB 外部 ID（例: '2019100001'）
    birth_year  INTEGER,
    father_name TEXT,
    created_at  TEXT
  );
  ```
- **統合ロジック**: 同名異形式（例: `ウシュバテソーロ` vs `ウシュバテソーロ(2019)`）を horse_id で統合。`merge_duplicate_horses.py --apply` で実行
- **残課題**: B_prefix 1,253 件の horse_id が未解決（詳細: `memory/future_tasks_horses_master.md`）
- **月次更新**: `build_horses_master.py --apply` → `merge_duplicate_horses.py --dry-run` の 2 ステップが月次チェックリスト
...
- **実行コマンド**: `powershell -ExecutionPolicy Bypass -File scripts\restart_backfill_b.ps1`
- **PID 管理**: `tmp/backfill_b.pid`。プロセス生存確認 → 存在すれば SKIP、なければ新規起動
- **ログ確認**: `Get-Content logs\backfill_b.log -Tail 20` で直近 20 行を確認
- **制約**: BOM 付き UTF-8 必須（Windows PowerShell 5.1 の Shift-JIS 解釈対策）。`-Encoding UTF8` オプション付きで保存されていること
...
- **辞書構造**: `Dict[str, str]` で `{競馬専門用語: 平易な日本語}` のマッピング。Qwen prompt 生成前にトークン置換を実施
- **対象パターン**: 「ひと開催あけて」「自分のリズムで運べれば」等のよくある曖昧表現を体言止め・言い切り形に変換
- **連携**: `scripts/local_llm_paraphrase.py` の prompt 生成フェーズで読み込み。辞書追加時は同スクリプトの `ASSERTIVE_RULES` セクションも確認
- **制約**: 辞書追加後は `scripts/verify_paraphrase.py` で品質確認必須
