# -*- coding: utf-8 -*-
"""真の M' 戦略 集計 (simulated skip 4 ルール再現 / 全期間反映)

【背景と目的】
  過去 pred.json (2024-01 〜 2026-03-11) には bet_decision フィールドが存在しない。
  diag_mprime_skip_aware.py は bet_decision が無い旧形式を
  「M' 戦略マッピングで E/F のみ skip」扱いにしており、実際には
  low_ev / dispersed / low_confidence / torigami による skip を再現できていない。
  → 過去期間の ROI が過大評価のまま。

  本スクリプトは betting.py の make_bet_decision (4 ルール) を
  pred.json のフィールドから完全再現し、全期間で真の skip 反映集計を行う。

【4 ルール (OR 発火 / skip=True)】
  Rule 1 low_ev:          confidence ∈ {B,C} かつ max(value_bets[i].ev) < 110
  Rule 2 dispersed:       len(tickets) > 20 かつ max(horses[i].place3_prob) < 0.15
  Rule 3 low_confidence:  confidence == "C" かつ ◎/◉ の win_prob < 0.15
  Rule 4 torigami:        tickets が存在 かつ buyable (stake>0) が 0件 かつ
                          skip_reason に "torigami" or "low_recovery" が含まれる

【bet_decision 既存データとの関係】
  - 2026-03-12 以降は pred.json に bet_decision フィールドが存在する
    → その値を優先使用 (既に正しい判定が入っているため)
  - 2024-01 〜 2026-03-11 は bet_decision なし
    → 4 ルールを pred.json フィールドから再現して判定

【value_bets.ev について】
  - 2024-01 〜 2026-03-11: value_bets フィールドなし → ev=0 扱い (= Rule 1 非発火)
    ※ 旧形式では odds/ev 計算自体が未実装
  - 2026-03-12 以降: bet_decision が存在するため simulated_skip は不使用
...
【除外ルール】
  ...
  - simulated_skip=True (または bet_decision.skip=True) の race は完全除外
  - ◎/◉ が存在しない race はスキップ

Usage:
  python scripts/diag_mprime_simulated_skip.py                          # フル実行
  python scripts/diag_mprime_simulated_skip.py --debug                  # 最初の 10 ファイル
  python scripts/diag_mprime_simulated_skip.py --start 2025-01-01 --end 2025-12-31
...
from config.settings import BET_DECISION_THRESHOLDS as TH

# ============================================================
# ロガー設定
# ============================================================
def _setup_logger(log_path: str) -> logging.Logger:
    logger = logging.getLogger("diag_mprime_simulated_skip")
...
DEFAULT_END   = "2026-05-30"  # exclusive
...
# BET_DECISION_THRESHOLDS (settings.py から取得済)
# th["low_ev_max_ev"]                    = 110
# th["dispersed_ticket_count"]           = 20
# th["dispersed_max_place3"]             = 0.15
# th["low_confidence_honmei_winprob"]    = 0.15
...
# ============================================================
# simulated_skip: betting.py の make_bet_decision を再現
# ============================================================
def simulated_skip(race: Dict) -> Tuple[bool, str]:
    """make_bet_decision の 4 ルールを pred.json フィールドから再現する

    Rule 1 low_ev:          confidence ∈ {B,C} かつ max_ev < 110
    Rule 2 dispersed:       len(tickets) > 20 かつ max(place3_prob) < 0.15
    Rule 3 low_confidence:  confidence == "C" かつ ◎/◉ の win_prob < 0.15
    Rule 4 torigami:        tickets あり・buyable なし・skip_reason に torigami/low_recovery
    ...
    tickets    = list(race.get("tickets", []) or [])
    horses     = list(race.get("horses",  []) or [])
    value_bets = list(race.get("value_bets", []) or [])

    # Rule 1: low_ev
    # value_bets[i].ev から最大 ev を取得 (tickets.ev は現存しない)
    max_ev = max((float(vb.get("ev", 0) or 0) for vb in value_bets), default=0.0)
    if confidence in ("B", "C") and max_ev < TH["low_ev_max_ev"]:
        return True, "low_ev"

    # Rule 2: dispersed
    max_p3 = max(
        (float(h.get("place3_prob", 0.0) or 0.0) for h in horses),
        default=0.0,
    )
    if len(tickets) > TH["dispersed_ticket_count"] and max_p3 < TH["dispersed_max_place3"]:
        return True, "dispersed"

    # Rule 3: low_confidence
    if confidence == "C":
        honmei = next(
            (h for h in horses if (h.get("mark") or "").strip() in ALLOWED_COL1_MARKS
             and not h.get("is_scratched")),
            None,
        )
        honmei_wp = float(honmei.get("win_prob", 0.0) or 0.0) if honmei else 0.0
        if honmei_wp < TH["low_confidence_honmei_winprob"]:
            return True, "low_confidence"

    # Rule 4: torigami
    buyable = [t for t in tickets if (t.get("stake", 0) or 0) > 0]
    if tickets and not buyable:
        skip_reasons = {t.get("skip_reason") for t in tickets if t.get("skip_reason")}
        if "torigami" in skip_reasons or "low_recovery" in skip_reasons:
            return True, "torigami"
...
def run(
    ...
    """4 馬券並列集計 (simulated skip=False のみ)

    Returns:
        (conf_agg, monthly_agg, counters, skip_reason_agg)
    """
    # 集計初期化
    ...
    # skip 理由別カウンタ
    skip_reason_agg: Dict[str, int] = {
        "low_ev": 0,
        "dispersed": 0,
        "low_confidence": 0,
        "torigami": 0,
        "e_f_skip": 0,        # E/F confidence による M' 戦略レベル skip
        "bet_decision_skip": 0,  # 既存 bet_decision.skip=True (2026-03-12 以降)
    }
    ...
    cnt_banei           = 0   # ばんえい除外
    cnt_skip_total      = 0   # 全 skip 数 (simulated + bet_decision)
    ...
    logger.info("  【simulated skip 4 ルール再現】bet_decision なし旧形式にも skip 判定を適用")
    ...
    for file_idx, fp in enumerate(pred_files):
        ...
        if file_idx % 50 == 0:
            ...
            logger.info(
                ...
                f"played={cnt_played:,} skip_total={cnt_skip_total:,} "
        ...
        for race in races:
            ...
            if not venue_code:
                race_id_str_tmp = str(race.get("race_id", ""))
                if len(race_id_str_tmp) >= 6:
                    venue_code = race_id_str_tmp[4:6]
            ...
            if bet_decision and isinstance(bet_decision, dict) and "skip" in bet_decision:
                # 2026-03-12 以降: 既存 bet_decision を信頼
                bd_skip = bet_decision.get("skip", False)
                if bd_skip:
                    should_skip = True
                    skip_reason_str = str(bet_decision.get("reason", "bet_decision"))
                    skip_reason_agg["bet_decision_skip"] += 1
            else:
                # 旧形式 (bet_decision なし): simulated skip 4 ルールを適用
                # まず M' 戦略レベルの skip (E/F) を先に判定
                if STRATEGY_M_PRIME.get(confidence) is None:
                    should_skip = True
                    skip_reason_str = "e_f_skip"
                    skip_reason_agg["e_f_skip"] += 1
                else:
                    # 4 ルール再現
                    should_skip, skip_reason_str = simulated_skip(race)
                    if should_skip and skip_reason_str in skip_reason_agg:
                        skip_reason_agg[skip_reason_str] += 1
    ...
    logger.info(f"  全 skip 除外:                     {cnt_skip_total:,}  ← simulated+bet_decision 合計")
    logger.info(f"    内訳 low_ev:                     {skip_reason_agg['low_ev']:,}")
    logger.info(f"    内訳 dispersed:                  {skip_reason_agg['dispersed']:,}")
    logger.info(f"    内訳 low_confidence:              {skip_reason_agg['low_confidence']:,}")
    logger.info(f"    内訳 torigami:                    {skip_reason_agg['torigami']:,}")
    logger.info(f"    内訳 e_f_skip (E/F conf):         {skip_reason_agg['e_f_skip']:,}")
    logger.info(f"    内訳 bet_decision_skip (既存):    {skip_reason_agg['bet_decision_skip']:,}")
    ...
    counters = {
        ...
        "skip_total":        cnt_skip_total,
        "no_conf":           cnt_no_conf,
        "no_marks":          cnt_no_marks,
        "no_results":        cnt_no_results,
        "played":            cnt_played,
    }
    return conf_agg, dict(monthly_agg), counters, skip_reason_agg
...
def write_summary_csv(
    ...
    header = [
        "confidence", "simulated_skip_strategy",
    ...
    for conf in CONFIDENCE_ORDER:
        ...
        rows.append({
            "confidence":              conf,
            "simulated_skip_strategy": strategy,
            "race_played":             rp,
...
def write_monthly_csv(
    ...
    header = [
        "yyyymm", "confidence", "simulated_skip_strategy",
        "race_played",
...
def write_skip_reasons_csv(
    ...
    header = ["skip_reason", "race_count", "pct_of_total"]
    rows = []

    total_skip = sum(skip_reason_agg.values())
    for reason, cnt in sorted(skip_reason_agg.items(), key=lambda x: -x[1]):
        rows.append({
            "skip_reason":    reason,
            "race_count":     cnt,
            "pct_of_total":   f"{cnt / max(total_races, 1) * 100:.2f}",
        })
    # 合計行
    rows.append({
        "skip_reason":    "TOTAL_SKIP",
        "race_count":     total_skip,
        "pct_of_total":   f"{total_skip / max(total_races, 1) * 100:.2f}",
    })
    ...
    logger.info(f"skip 理由 CSV 出力: {output_path} ({len(rows)} 行)")
...
def print_report(
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("【真の M' 戦略 集計結果 (simulated skip 4 ルール再現 / 4 馬券並列)】")
    logger.info("  旧形式 pred.json に make_bet_decision 相当の skip 判定を retroactive 適用")
    logger.info("=" * 80)
    logger.info(f"  総 race 数:                      {counters['total_races']:,}")
    logger.info(f"  ばんえい除外:                     {counters['banei_skip']:,}")
    logger.info(f"  全 skip 除外:                     {counters['skip_total']:,}")
    logger.info(f"    内訳 low_ev:                     {skip_reason_agg['low_ev']:,}")
    logger.info(f"    内訳 dispersed:                  {skip_reason_agg['dispersed']:,}")
    logger.info(f"    内訳 low_confidence:              {skip_reason_agg['low_confidence']:,}")
    logger.info(f"    内訳 torigami:                    {skip_reason_agg['torigami']:,}")
    ...
    logger.info("")
    logger.info(
        f"  {'馬券':14s} | {'played':>7s} | {'hit':>6s} | {'hit%':>7s} | "
        f"{'投資':>12s} | {'払戻':>12s} | {'収支':>12s} | {'ROI%':>8s}"
    )
    logger.info("  " + "-" * 90)
    ...
    def _fmtrow(bet: str, p: int, h: int, stk: int, pay: int) -> str:
        ...
        bal = pay - stk
        return (
            f"  {bet:14s} | {p:>7,} | {h:>6,} | {hp:>6.1f}% | "
            f"{stk:>12,} | {pay:>12,} | {bal:>+12,} | {roi:>7.1f}%"
        )
    ...
    for conf in CONFIDENCE_ORDER:
        ...
        if strategy == "SKIP":
            logger.info(f"  {conf:4s} | [SKIP] simulated_skip=True が大半 (E/F confidence)")
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "真の M' 戦略 集計 (simulated skip 4 ルール再現 / 全期間反映)\n"
            "旧形式 pred.json に betting.py make_bet_decision 相当の skip 判定を retroactive 適用"
        )
    )
    ...
    log_path = os.path.join(DIAG_DIR, "mprime_simulated_skip_run.log")
    logger   = _setup_logger(log_path)

    logger.info("=== diag_mprime_simulated_skip.py 開始 ===")
    logger.info("  【simulated skip 4 ルール再現】")
    logger.info("  Rule 1 low_ev:         conf={B,C} かつ max(value_bets.ev) < 110")
    logger.info("  Rule 2 dispersed:      len(tickets) > 20 かつ max(place3_prob) < 0.15")
    logger.info("  Rule 3 low_confidence: conf==C かつ honmei.win_prob < 0.15")
    logger.info("  Rule 4 torigami:       tickets あり・buyable=0 かつ skip_reason に torigami/low_recovery")
    logger.info(f"  期間: {args.start} 〜 {args.end} (exclusive)")
    logger.info(f"  デバッグ: {args.debug}")

    # 集計実行
    conf_agg, monthly_agg, counters, skip_reason_agg = run(
        args.start, args.end, args.debug, logger
    )

    # CSV 出力
    summary_path      = os.path.join(DIAG_DIR, "mprime_simulated_skip_summary.csv")
    monthly_path      = os.path.join(DIAG_DIR, "mprime_simulated_skip_monthly.csv")
    all_periods_path  = os.path.join(DIAG_DIR, "mprime_simulated_skip_all_periods.csv")
    skip_reasons_path = os.path.join(DIAG_DIR, "mprime_simulated_skip_reasons.csv")
    ...
    write_skip_reasons_csv(skip_reason_agg, counters["total_races"], skip_reasons_path, logger)

    # 最終レポート
    print_report(conf_agg, counters, skip_reason_agg, master_total, logger)
    ...
    logger.info(f"  skip_reasons CSV: {skip_reasons_path}")
