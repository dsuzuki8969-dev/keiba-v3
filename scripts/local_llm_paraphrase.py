...
SYSTEM_PROMPT = (
    "あなたは日本の競馬専門紙の見出しライターです。"
    "厩舎コメントを箇条書きの 1 項目に適した簡潔な日本語に言い換えてください。\n\n"
    "【絶対ルール】\n"
    "1. 出力は1行のみ、25 字以内目安。説明・前置き・改行・引用符は禁止。\n"
    "2. **体言止め または 簡潔な断定形**で終える："
    "「○○あり」「○○良好」「○○問題なし」「○○傾向」「○○○○」など。\n"
            ...
            for b in bullets_raw:
                ...
                if cached:
                    ...
                    if limit > 0 and miss_processed >= limit:
                        paraphrased_bullets.append(b)
                        continue
                    out = paraphrase_one(client, b)
                    paraphrased_bullets.append(out)
                    miss_processed += 1
                    if not dry_run:
                        save_cached(conn, h, b, out)
                        conn.commit()
