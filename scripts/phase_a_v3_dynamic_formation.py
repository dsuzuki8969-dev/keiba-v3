"""Phase A v3 — 動的フォーメーション + ☆補完 三連複 ROI バックテスト
(マスター仕様 T-049 / 2026-04-29 確定)

戦略 (動的フォーメーション、3ケース判定):
  絞り (S-strict) : ◉◎.ev>=1.8 AND ◉◎.place3_prob>=0.65 AND ○.place3_prob>=0.50
                    → ◉◎-○ → {▲△★☆} (3着) = 4点
  中   (S-mid)    : ◉◎.ev>=1.3 AND ◉◎.place3_prob>=0.55 AND (○.place3_prob>=0.40 OR ▲.place3_prob>=0.40)
                    → ◉◎ → {○,▲} → {○,▲,△,★,☆} 重複除外 = 7点
  広   (S-wide)   : ◉◎.ev>=1.0 (デフォルト)
                    → ◉◎ → {○▲△★☆} 5頭から2頭組合せ = 10点
  見送り          : ◉◎.ev < 1.0
                    → 買わない = 0点

☆補完: ☆印なし → 無印馬 (odds>=10.0) の win_prob 最高位 1頭を動的☆扱い
        補完不可 → 5頭流しから4頭流しに縮退 (広: 10点→6点)

使い方:
  python scripts/phase_a_v3_dynamic_formation.py \\
      --start-date 20240101 \\
      --end-date   20260428 \\
      --output-csv tmp/phase_a_v3_results.csv \\
      --output-json tmp/phase_a_v3_results.json \\
      --output-log tmp/phase_a_v3_execution.log
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase A v3 — 動的フォーメーション + ☆補完 三連複 ROI バックテスト"
    )
    ...
    parser.add_argument("--output-csv",  default="tmp/phase_a_v3_results.csv")
    parser.add_argument("--output-json", default="tmp/phase_a_v3_results.json")
    parser.add_argument("--output-log",  default="tmp/phase_a_v3_execution.log")
    ...
    logger.info("Phase A v3 バックテスト開始: %s〜%s (%d 日間)", s, e, total_days)
    ...
    logger.info("")
    logger.info("[████████████████████████████████████████] 100%% | T-049 Phase A v3 動的フォーメーション全期間集計完了 | 累犯:14")
    logger.info("出力ファイル:")
