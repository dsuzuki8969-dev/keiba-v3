# -*- coding: utf-8 -*-
"""NAR基準タイム変更 前後比較スクリプト

変更前(before_basetime)と変更後のpred.jsonを比較し、
◎的中率・ROI・印変化率を集計する。
"""
import json
import os
...
def main():
    ...
    # before_basetime ファイルを持ち、かつ pred.json が再分析済み(更新済み)の日付を自動検出
    before_files = sorted(PRED_DIR.glob("*_pred_before_basetime.json"))
    dates = []
    skipped = []
    ...
    for date in dates:
        old_path = PRED_DIR / f"{date}_pred_before_basetime.json"
