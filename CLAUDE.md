...
- **プログレスバー＆タスクリストを Chat 本文に必ず可視化**（**違反歴 3 回・最重要**）。詳細は `feedback_progress_bar.md`
  ...
  - **subagent 待機中・Bash 実行中もバー必須**（投げっぱなし禁止。2026-04-27 違反）
  - **ログを `tail -2` で切り捨てない**（思考共有のため最低 `tail -8` or 完全表示。2026-04-27 違反）
