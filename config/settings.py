...
DEVIATION = {
    ...
    # 騎手・調教師の偏差値レンジ（ベイズ収縮付き勝率ベース、50 中心）
    # ability とは別概念: ベイズ収縮により極端な値は出にくいため 20-100 で十分
    "personnel": {"min": 20, "max": 100},
}

# ============================================================
# Plan-γ: ハイブリッド相対偏差値（Phase 1 設定値）
# ============================================================
# z-score 正規化の標準偏差下限（小レース・同タイム多数時の安定化）
RELATIVE_DEV_SIGMA_FLOOR: float = 5.0
# z-score クランプ幅（±3σ = 偏差値 20〜80 に収まる）
RELATIVE_DEV_Z_CLAMP: float = 3.0
# 帯広ばんえい venue_code
BANEI_VENUE_CODE: str = "65"
# field_count < MIN でバックフィルをスキップ（relative_dev = NULL のまま）
RELATIVE_DEV_MIN_FIELD: int = 5

# ============================================================
# Plan-γ Phase 3: ハイブリッド合算設定
# ============================================================
# True で印付与の判定値を hybrid_total に切替（本番切替はバックテスト検証後に行うこと）
USE_HYBRID_SCORING: bool = False
# ハイブリッド重み β: hybrid = ability_total*(1-β) + race_relative_dev*β
# β=0.0: 絶対指標のみ / β=0.30: 絶対70%+相対30% / β=1.0: 相対指標のみ
HYBRID_BETA: float = 0.30
...
# 4パターン formation 決定木（2026-06-22 マスター承認・旧5パターン A/C/B 置換）
# g1=軸-○, g2=○-▲, g3=▲-△。決定木:
#   g1<KIKKO → ③二頭拮抗 / g1≥GAP → ④1頭流し / g2≥GAP → ①2頭流し /
#   g3≥GAP → ②相手拮抗 / else → 見送り
...
# ML印転写クランプ (roadmap P2): engine.py Step5.6 で win_prob を composite に反映する際の
# 上限幅(±pt)。5.0=現状維持(本番デフォルト) / 拡大でML信号の印への影響を強化。
# 環境変数 ML_ADJ_CLAMP=10 等で WF 効果測定用に変更 (本番は未設定=5.0 で挙動不変)。
ML_ADJ_CLAMP = float(os.environ.get("ML_ADJ_CLAMP", "5.0"))

# 最終確率のメリハリ拡大: べき乗変換（1.0=変更なし, >1.0で差を拡大）
# 正規化後に適用するため、合計値は保たれつつ上位がより突出する
PROB_SHARPNESS = 1.45  # キャリブレーション修正: 旧1.30→1.45（上流圧縮削減に伴い強化）

# 表示勝率シャープ化（2026-06-22 マスター承認・pred後処理）
# メリハリ拡大（べき乗γ）+ 圧倒的人気の安泰下限。表示のみ・印/composite/買い目ロジックは不変。
# scripts/sharpen_win_prob_display.py が pred.json 後処理で適用。
DISPLAY_PROB_GAMMA = 2.2  # シャープ化指数（大きいほど本命集中・1.0=無変換）
# 本命安泰下限: (オッズ上限, 表示勝率下限)。元win上位3位以内の馬のみ適用（市場非参照の理念ガード）。
DISPLAY_PROB_ODDS_FLOORS = [
    (1.3, 0.62),
    (1.7, 0.56),
    (2.2, 0.50),
    (3.0, 0.43),
...
# v2 (2026-05-20): engine.py内のPostCalibratorは無効化
# 代わりに scripts/recalibrate_win_prob.py でpred.jsonに直接適用
# 理由: engine.pyのcalibratorは正規化前raw値に適用されるが、
#        学習データは正規化後の値 → 分布不一致で性能悪化していた
USE_POST_CALIBRATOR = False
...
# ============================================================
# M-1: 学習リーク防止 — pred.json 生成タイムスタンプ検証
# (2026-05-27 追加: L-1 学習リーク真因確定 → 再発防止措置)
# ============================================================
# _leak_risk_flag = "WARN" とする閾値: race_date から何日後に生成したか
LEAK_RISK_DAYS_WARN: int = 3     # 3日以内: WARN (race直後の補完再生成、許容範囲)
# _leak_risk_flag = "DANGER" とする閾値: これ以上後で生成 = 学習リーク疑い濃厚
LEAK_RISK_DAYS_DANGER: int = 4   # 4日以上後: DANGER (結果学習済モデルで後追い予想)
# 既存 pred.json がある場合に再生成を警告する閾値 (日数)
# N_DAYS_LOCK 日以上後の再生成は --allow-past-regen フラグなしで拒否
LEAK_RISK_LOCK_DAYS: int = 3     # race_date + 3日超過で再生成ロック

# 成績集計のリーク無 起点 (2026-07-01 調査で確定):
# 2025〜2026-01 の pred.json は全て race後(数ヶ月〜1年+)に再生成された学習リーク版で、
# 複勝率が72〜79%に水増しされていた (live生成分のみのプールは複勝57.9%)。
# 「全期間」表示の主要KPIは実力を約+10pt過大評価するため、この日付以降のみで集計する。
# 年別(2024/2025/2026)の明示指定は歴史参照用にそのまま集計する (opt-in)。
RESULTS_LEAK_FREE_START: str = "2026-02-01"

# ============================================================
# P3: ダート道悪×脚質 馬場補正フラグ (2026-06-28 追加)
# ============================================================
...
# → 偏差値スケールに控えめ変換: 逃げ+3.0 / 先行 0.0 / 差し-1.5 / 追込-1.5
# 芝・良馬場・稍重は補正なし（芝は根拠弱・稍重は中間で除外）
BABA_PACE_DIRT_BAD: dict = {
    "逃げ":  3.0,
    "先行":  0.0,
    "差し": -1.5,
    "追込": -1.5,
