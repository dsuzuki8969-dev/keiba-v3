...
- **カレンダー●** 単勝収支→**◎複勝率(的中)ベース**(dot 3段・tooltip 勝/連対/複勝)。閾値は直近リーク無レジーム分位点。
- **🚨リーク確定**: pred mtime実測で2025〜2026-01は全てレース後に再生成=学習リーク。live生成分のみプール=**複勝57.9%** vs regen68.2%(同一月2026-05でlive55.1% vs regen61.6%=+6.5pt水増し)。
- **KPIリーク無化(B)**: `RESULTS_LEAK_FREE_START="2026-02-01"`。`aggregate_all`/`aggregate_detailed`の全期間(year="all")は起点以降のみ集計。年別はopt-in。frontend「全期間(リーク無)」+注記。
- **効果(実画面確認済)**: 全期間 勝率36.1→**30.5%** 連対55.8→**49.9%** 複勝68.0→**62.3%**(2026-02〜6693R)。dashboard再起動+cache再削除で新コード反映済。
- **残**: より厳密なleak-free実力は複勝~58%(live only)。B=62%は2026-02〜04にregen微残。dashboard自己再起動口(`/api/admin/restart`)未実装(インフラの壁)。
...
> **タブ連動(2026-06-30・master指示・commit c298e7b)**: 詳細分析の全体/JRA/NARタブ切替で上部の本命的中カード(勝率/連対率/複勝率)も連動。`cat`状態をResultsPageへ持ち上げ→SummaryCardsが `detailed[cat].stats` の本命集計を読む(backend不要・frontendのみ)。カードラベルに(全体/JRA/NAR)表示。全体は36.1→36.2%(detailed基準=帯広除外整合)。実画面で3カテゴリ連動確認。
> **競馬場連動(2026-06-30・master指示・commit 1340cb7)**: 競馬場ボタン選択でも本命カードがその場の成績に連動。`selectedVenue`をResultsPageへ持ち上げ→SummaryCardsが `detailed[cat].by_venue[venue]` を読む。ラベルに競馬場名(例:園田)。再クリックで全体へ。全24場・実画面で園田(勝38.7/複70.8%)確認。**全 push 済 origin/master=1340cb7**。
