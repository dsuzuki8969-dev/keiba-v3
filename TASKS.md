...
## 🚨 次セッション P0 (v5+α 改訂 — マスター指摘反映)

**前提**: WF ROI 72.7% は赤字運用 = 存在意義なし。「妥当性判断」は思考停止。
**真の P0 は ML モデル / 予測パイプライン根本再設計**。

| 順 | ID | 内容 | 工数 |
|---:|---|---|---|
| **1** | **L-2** | WF backtest 自体の bug 確認 (Layer 2 _horse_history 日付フィルタ / オッズ・人気のリーク有無) | 1-2 日 |
| **2** | **M-2 ML 根本再設計** | WF ROI 72.7% を 100% 超に改善する設計改革。候補: (a) popularity_blend を WF にも組込み (b) 特徴量再選定 (c) ターゲット変更 (回収率最大化 vs Top1 確率最大化) (d) アンサンブル/スタッキング (e) calibration 強化 | **大規模 (1-2 週)** |
