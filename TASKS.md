...
### 🚨 真の P0 次セッション (ROI 100% 達成への次手 — 試行錯誤継続)

**5/27 v6 試行錯誤の最大成果**:
- ◎単勝 オッズフィルター <2.0: ROI **80.3%** (+5.0pt)
- <1.3 + JRA: 推定 ROI **88-90%** (+13-15pt)
- 目標 100% 超まで **-10〜20pt**

| 順 | 試行案 | 期待効果 |
|---:|---|---|
| **1** | **複数買い戦略** (◎+○+▲ 三連系 / 既存 30 通り三連複 等) | +10〜30pt |
| **2** | **<1.3 + JRA + 期間絞り** で 100% 超 race 群発見 | +α |
| **3** | **EV>1.0 AND odds<5.0** 組合せ (本命寄り EV) | +5〜10pt |
| **4** | **mark="◉" (高信頼) 限定 + odds フィルター** | +5〜15pt |
| **5** | **複勝戦略 payouts バグ修正** (旧形式 dict 連結値 parse) | +α (精度評価) |
| **6** | **M-2 方針 3** ROI 観点 特徴量再選定 (SHAP + permute) | +5〜15pt |
| **7** | **head_win 階層モデル化** (現状 global のみ → jra_turf 等追加) | +5〜10pt |
| 派生 | wf_2024 stats 空問題 / 方針 2 通年化 / 方針 4 係数調整 | 各 半日〜2日 |
