...
## 📋 5/25 セッション完了 (5/24 引継ぎ 28 件 / 完走)

> 5/24 セッションの 28 件 + 本日発見 1 件 = 計 29 件を本日マスター承認パスで完走。
> マスター指示「もう寝る時間だから承認なしで君の推奨で完走しておいて」(5/25 早朝)。
> 詳細は `memory/handoff_2026-05-25.md` (本日作成) と git log 参照。

### 🔴 残課題 (本日発見・次セッション P0)

| ID | 内容 | 工数 | 経緯 |
|---|---|---|---|
| **C-1b** | verify_all_tickets vs analyze_r1 残乖離 ~18pt 真因究明 | 30 分 | C-1 で tickets/tbm_fixed 不整合解消 + verify stake bug 修正後も verify ALL 三連複 ROI 129.0% vs R-1 三連複合計 147.9%。同着 (multi-entry payout) / per-race vs per-ticket 集計方式差が候補 |
| **A-3 監視** | Walk-Forward 学習 進捗確認 | 5 分 | バックグラウンド起動済 (5/25 早朝)。完了予定: 数時間〜数日。data/models/wf_{2024,2025,2026}/ 生成確認 |

### 🟠 保留タスク (本日マスター判断で保留 = A-1 完了後に再判断推奨だったが完走後の数字確認できず)

| ID | 内容 | 推奨着手タイミング |
|---|---|---|
| **A-4** | 金沢 -41.1pt 悪化 個別調査 | A-3 WF 完了後 (Phase 反映で改善期待) |
| **D-7** | bet_decision skip 強化 | C-1b 真因確定後 |
| **D-8** | ◎単勝オッズ 下限ガード (1.5 倍以下 skip) | D-1 採用後の新データで再評価 |
| **B-2** | △★☆ の役割再定義 (買い目で使わない) | A-3 完了後の新分布で再評価 |
| **B-3** | composite 重み ROI ベース再較正 | A-3 と統合 |
| **B-4** | ◎ハズレ時 無印 20.3% 救済策 | A-3 完了後の残無印率で再判断 |
| **D-3** | M' 戦略 点数最適化 (SS/S/A/B/C/D) ROI 主義で再較正 | A-3 完了後 |
| **D-4** | 複勝・馬連・ワイド導入検討 | C-3 R-1 結果ベース (本日得た) で判断可能 |
| **D-6** | 期待値ベース買い目選定 | A-3 + D-1 適用後 |
| **E-6** | horse_sire_map.pkl 空問題 | A-3 WF 再構築後 |
| **G-2** | 会場別の買い目戦略変更 (金沢含む) | A-1 + A-4 後 |
| **G-3** | 回収率モニタリング dashboard 強化 | 6 月実運用後 |

### 🟢 P3 (低優先 / 自動待ち)

| ID | 内容 | 状態 |
|---|---|---|
| D-5 | 三連単高配当狙い | 棚上げ (6 月実運用安定後) |
| E-3 | NAR L (2201-2600m) backfill | 自然待ち (サンプル蓄積) |
...
| **F-1** | analyze_r1 スクリプトコミット (既 ccc49e6 で完了確認) | — |
...
| **C-1c** | verify_all_tickets 三連複専用集計 bug 修正 (-34pt 改善) | d8e4dee |
| **A-1** | 全 pred.json regen (873 日 / 90 秒) | データ更新のみ |
...
| **E-1+E-2** | frontend 再ビルド (4.03秒) + src/static 同期 + dashboard smoke OK | static |
| **E-5** | TASKS.md 整理 (本作業) | 本 commit |
| **A-3** | Walk-Forward 学習 バックグラウンド起動 (進行中・別セッションで確認) | log/wf_train_20260525.log |

### 📝 棄却

| ID | 内容 | 理由 |
|---|---|---|
| G-1 | 印の表示順序最適化 (composite 順 vs ROI 寄与順) | 現状 composite 順は「印=実力評価」哲学に一致・却下 |
