...
### 🚨 真の P0 次セッション (M-2 本格実装フェーズ)

| 順 | ID | 内容 | 工数 |
|---:|---|---|---|
| **1** | **M-2 方針 4** | 期間別 popularity_blend (`popularity_rates_wf_*.json`) 実装 → WF 再評価 | 3 日 |
| **2** | **M-2 方針 2** | WF 内部 Isotonic calibration 統合 → WF 再評価 | 3 日 |
| **3** | **M-2 方針 1** | ML 学習目的の二段化 (head_top3 + head_win) → WF 再評価 | 1 週 |
| **4** | **M-2 方針 3** | ROI 観点 特徴量再選定 (SHAP + permute ROI 寄与度) | 1 週 |
| 合計 | | 全方針実装 + 統合 WF backtest + 凍結戦略再評価 | **2-3 週 / 7-11 セッション** |
