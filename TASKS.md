...
### T-050 + T-052 + T-053 (P0・進行中) — 3 券種ハイブリッド振り分け方式 (案 C 確定)
- **戦略進化** (2026-04-30 マスター承認): Phase A 全件併用 → **「振り分け方式 案 C ハイブリッド最適化」** へ進化
  - マスター指示「どれを選ぶか + どれを組み合わせるか」 → 2³=8 通りの組合せから動的選択
  - 案 C = Layer 1 独立 EV 判定 + Layer 2 5 ルール組合せ + Layer 3 信頼度ガード
- **plan**: `~/.claude/plans/claude-claude-md-http-claude-md-keiba-v-wiggly-cake.md` (案 C 確定設計)
- **マスター回答 (2026-04-30)**:
  - 判定ロジック: **案 C ハイブリッド最適化**
  - 信頼度ガード (SS/C/D skip): **バックテスト結果次第**
  - 本番適用タイミング: **バックテスト結果後に判断**
- **本セッション進行中** (Opus 自走):
  - Sonnet A: バックテスト 6 ケース実行 (C-S/C-SA/C-NONE/A-S/A-SA/A-NONE × 846 日)
  - Sonnet B: T-047 構造修正 (Flask background thread)
- **完了基準**: 6 ケース ROI 比較 → 採用ケース確定 → 案 C 本番実装 plan 化 (次セッション)
- **参考期待効果** (Phase A v3-fix=A-NONE 全件併用): 年純利 +862 万円
  - 三連複動的: +662 万 (5,787 R / ROI 242.1%)
  - 馬単中拡張: +38 万 (735 R / ROI 173.7%)
  - 単勝 T-4: +162 万 (26,736 R / ROI 130.3%)
