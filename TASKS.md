...
## 📋 5/24 セッション 次セッション引継ぎ (28 件・優先度付き)

> 本日 5/24 セッション (7 commit 完了) で出た改善案・調査事項。マスター指示「全 28 件を次セッションに引継ぎ」(5/24 23:30)。
> 詳細は `memory/handoff_2026-05-24.md` 参照。

### 🔴 P0 (最優先・即着手すべき)

| ID | 内容 | 工数 | 期待効果 |
|---|---|---|---|
| **C-1** | verify_all_tickets vs R-1 の **37.3pt ROI 乖離** 真因調査 | 30 分 | 真の三連複 ROI 確定。他の判断の前提 |
| **D-1** | **JRA 単勝 戦略B (shobu_score TOP2) 採用** — engine.py + batch_wf_fast.py で生成 | 30 分 | **ROI 144.5% 機会損失救済** (戦略B シミュ済) |
| **B-1** | ML 合議 (win_prob 1位 → ◎ 昇格) 削除 | 30 分 | 効果ゼロ実証済 (乖離 8,156 R で 23.5% vs 23.3%) |
| **C-3** | R-1 他券種 (複勝・馬連・馬単・ワイド・三連単) のバグ確認 | 30 分 | 集計信頼性確保 |
| **F-1** | R-1 スクリプト (`scripts/analyze_r1_ticket_roi.py`) コミット | 5 分 | 単勝/payout 形式バグ修正の永続化 |

### 🟠 P1 (高優先・1-2 日以内)

| ID | 内容 | 工数 | 期待効果 |
|---|---|---|---|
| **A-1** | 全 pred.json 印・買い目 `regen_strategy` で一括再生成 | 1 時間 | 整合性回復・真の Phase 効果測定の前提 |
| **A-2** | A-1 後の `verify_all_tickets` 再実行 | 5 分 | 真の全期間 ROI 確定 |
| **A-4** | 金沢 -41.1pt 悪化 個別調査 (Phase 改善過剰補正の疑い) | 半日 | 個別救済 |
| **D-7** | bet_decision skip 強化 (より多く skip して ROI 改善) | 半日 | 自信ない買い目除外 |
| **D-8** | ◎単勝オッズ 下限ガード (例: 1.5 倍以下 skip) | 30 分 | 配当伸びないレース除外 |
| **F-2** | payout 形式統一 (JRA dict / NAR list 混在を正規化) | 2 時間 | 集計バグの恒久対策 |
| **E-1** | 5/24 dashboard 実画面確認 (人間目視) | 5 分 | 本日成果の動作確認 |
| **E-5** | TASKS.md 整理 + 残課題棚卸し | 30 分 | プロジェクト管理 |

### 🟡 P2 (中優先・数日規模)

| ID | 内容 | 工数 | 期待効果 |
|---|---|---|---|
| **A-3** | **Walk-Forward モデル再構築 + Phase 1-3 反映** (handoff E 計画) | 数日 | **2024-2025 ROI 改善 (本命)** |
| **B-2** | △★☆ の役割再定義 (買い目で使わない・印自体は維持) | 1 時間 | 馬券点数効率化 |
| **B-3** | composite 重み ROI ベース再較正 | 数日 | 会場別最適化 |
| **B-4** | ◎ハズレ時 無印 20.3% 救済策 (composite 計算見直し) | 1-2 日 | ◎ハズレ救済 (印拡張は却下済) |
| **C-2** | 2026 単勝・複勝・馬連等の欠損調査 + backfill | 1 時間 | データ品質 |
| **D-3** | M' 戦略の点数最適化 (SS=4/S=7/A=7/B=10/C=10/D=10) ROI 主義で再較正 | 1 日 | 点数効率化 |
| **D-4** | 複勝・馬連・ワイド導入検討 (現在は単勝+三連複のみ) | 1-2 日 | 券種拡充 |
| **D-6** | 期待値ベース買い目選定 (現在は composite 順位ベース) | 数日 | 配当期待値最大化 |
| **E-2** | frontend/dashboard 再ビルド (5/22 night + 5/24 Phase 改善反映) | 30 分 | 表示最新化 |
| **E-6** | handoff_2026-05-13 残課題 (horse_sire_map.pkl 空問題) | 半日 | 既存課題消化 |
| **G-1** | 印の表示順序最適化 (composite 順 vs ROI 寄与順) | 1 日 | 表示改善 |
| **G-2** | 会場別の買い目戦略変更 (金沢 -41pt 悪化対策含む) | 1-2 日 | 会場別最適化 |
| **G-3** | 回収率モニタリング ダッシュボード強化 (リアルタイム ROI 表示) | 1-2 日 | 監視性向上 |

### 🟢 P3 (低優先・数週〜数ヶ月)

| ID | 内容 | 工数 | 備考 |
|---|---|---|---|
| **D-5** | 三連単高配当狙い (現在は三連複のみ) | 1 日 | 機会拡充 |
| **E-3** | NAR L (2201-2600m) backfill (現状 597 件→4,000 必要) | 数週 | サンプル増 |
| **E-4** | 6 月実運用での Phase 効果測定 (実レース) | 1-2 週 | 自然待ち |

### 📝 既決定 (本日完了)

| ID | 内容 | 状態 |
|---|---|---|
| D-2 | NAR 単勝 集計バグ修正 (R-1 内部) | ✅完了 (本日内) |
