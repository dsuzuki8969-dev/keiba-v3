...
### 🚨 真の P0 次セッション (M-2 本格実装 残 2 方針)

| 順 | ID | 内容 | 工数 |
|---:|---|---|---|
| **1** | **M-2 方針 1** | ML 学習目的の二段化 (head_top3 + head_win) → WF 再評価 | 1 週 / 2-3 セッション |
| **2** | **M-2 方針 3** | ROI 観点 特徴量再選定 (SHAP + permute ROI 寄与度) | 1 週 / 2-3 セッション |
| 統合 | | 全方針 + WF backtest + 凍結戦略再評価 | 1 セッション |
| **派生課題** | wf_2024 stats 空問題 (2023 以前データ無し) | 解決可能なら results.json から popularity 集計 | 1-2 日 |
| **派生課題** | wf_2024 hit% +0.5pt の説明不能変化 (blend 適用なしのはず) | tracker 決定論性 / race 順序の調査 | 1 日 |
| **派生課題** | 方針 2 calibrator 学習期間最適化 (現在: train_max 直前 N=3000 race の秋競馬集中 → 通年化) | 改善余地 +α | 半日 |
| **派生課題** | 方針 4 blend 係数調整 (ALPHA_MODEL_MIN 0.95→0.85) | 改善余地 +α | 半日 |
