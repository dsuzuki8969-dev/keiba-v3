...
### 🚨 真の P0 次セッション (M-2 残 方針 3 + 構造修正再試行)

| 順 | ID | 内容 | 工数 |
|---:|---|---|---|
| **1** | **mark 統合 win_weight 増加** | 0.5 → 2.0 / 3.0 で再試行 (effect 検証) | 半日 / 1 セッション |
| **2** | **mark 直接置換** | composite TOP1 ではなく head_win TOP1 を ◎ に直接設定 | 半日 |
| **3** | **EV ベース買い目選定** | head_win × tansho_odds で EV 最大馬を ◎ に | 1 日 |
| **4** | **M-2 方針 3** | ROI 観点 特徴量再選定 (SHAP + permute) + FEATURE_COLUMNS_WIN | 1 週 / 2-3 セッション |
| **派生課題** | wf_2024 stats 空問題 / hit% 変化 / 方針 2 通年化 / 方針 4 係数調整 / 方針 1 階層化 | 各 半日〜2日 | +α |
