...
_AUTO_FETCH_COOLDOWN_SEC = 180              # 後方互換用エイリアス（_AUTO_FETCH_COOLDOWN_MIN と同値）
_AUTO_FETCH_COOLDOWN_MIN = 180             # exponential backoff の最小クールダウン (秒)
...
(function(){{
  ...
  if(anchor && anchor.nextElementSibling){{
            ...
            for race in pred.get("races", []):
                ...
                if not force:
                    ...
                    effective_cooldown = min(
                        _AUTO_FETCH_COOLDOWN_MIN * (2 ** fail_cnt),
    ...
    def _start_background_result_fetcher():
        ...
        while True:
            try:
                ...
                # pending 件数を事前算出してログに残す
                try:
                    from src.results_tracker import load_prediction
                    _pred = load_prediction(today_date)
                    _races = _pred.get("races", []) if _pred else []
                    _date_key = today_date.replace("-", "")
                    _rfp = os.path.join(PROJECT_ROOT, "data", "results", f"{_date_key}_results.json")
                    _existing: set = set()
                    if os.path.isfile(_rfp):
                        import json as _json
                        with open(_rfp, "r", encoding="utf-8") as _f:
                ...
                for _r3 in _races:
                    ...
                    def _make_timer_cb(d: str = today_date) -> None:
                        def _cb3() -> None:
                            threading.Thread(
    ...
    # _BG_FETCHER_STARTED で create_app() 複数呼び出し時の二重起動を防止
    global _BG_FETCHER_STARTED
    if not _BG_FETCHER_STARTED:
        _BG_FETCHER_STARTED = True
        threading.Thread(target=_start_background_result_fetcher, daemon=True, name="bg_result_fetcher").start()
        logger.info("[bg_fetcher] background_result_fetcher 起動 (10 分間隔)")
    else:
        logger.debug("[bg_fetcher] 既に起動済みのためスキップ")
