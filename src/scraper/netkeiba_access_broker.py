...
class NetkeibaAccessBroker:
    ...
    def __init__(
        ...
        # mypy 互換のため Optional[IO[str]] で型注釈 (None 推論を避ける)
        self._file_handle: Optional[IO[str]] = None  # portalocker が管理するファイルオブジェクト
    ...
    def acquire(self, timeout: Optional[float] = None) -> bool:
        ...
        with self._thread_lock:
            ...
            try:
                ...
                # "a" モードで開く: ファイル切り詰めを防ぐ (別プロセス保持中の内容破壊回避)
                self._file_handle = open(self._lock_file, "a", encoding="utf-8")  # noqa: SIM115 - portalocker 管理のため context manager 不可
                portalocker.lock(
                    self._file_handle,
                ...
                while time.time() < deadline:
                    time.sleep(0.1)
                    try:
                        # "a" モード: ファイル切り詰め防止 (別プロセス保持中の内容破壊回避)
                        self._file_handle = open(self._lock_file, "a", encoding="utf-8")  # noqa: SIM115 - portalocker 管理
                        portalocker.lock(
                            self._file_handle,
                            portalocker.LOCK_EX | portalocker.LOCK_NB,
                        )
    ...
    def __enter__(self) -> "NetkeibaAccessBroker":
        # context manager: acquire 失敗 (timeout / portalocker なし) でも続行するが必ず警告ログを出す
        # 排他制御失敗を呼び出し元が無視しないよう、is_locked() で判定可能
        acquired = self.acquire()
        if not acquired:
            logger.warning(
                "broker: context manager — ロック取得に失敗しましたがコンテキストを続行します。"
                " is_locked() で取得状態を確認できます: %s",
                self._lock_file,
            )
        return self
