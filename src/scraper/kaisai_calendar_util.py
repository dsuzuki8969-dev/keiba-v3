...
def _load_calendar_nolock() -> Optional[dict]:
    """kaisai_calendar.json を読み込む（初回のみ）。

    呼び出し側で _calendar_lock を保持していることが前提。
    ロックを取得しないため、add_venue_to_calendar 等の「読み込み→変更→保存」を
    単一ロック内で原子的に行いたいケースから利用する (TOC/TOU 回避)。
    """
...
def _load_calendar() -> Optional[dict]:
    """kaisai_calendar.json を読み込む（初回のみ・スレッドセーフ）。"""
    with _calendar_lock:
        return _load_calendar_nolock()
...
def add_venue_to_calendar(
    ...
    """
    # 読み込み→変更→保存を単一ロック内で原子的に行う (TOC/TOU 回避)。
    with _calendar_lock:
        data = _load_calendar_nolock()
        if data is None:
            return False
