...
class StandardTimeCalculator:
    ...
    VENUE_SPEED_TABLE: Dict[str, Dict[str, float]] = {
        ...
        # --- 施策#2: NAR ダート用 VENUE_SPEED_TABLE (race_log実データから算出) ---
        # 秒/200m単位。正=その場は遅い、負=速い。全NAR会場のグランド平均を基準(0)。
        "NAR_ダート": {
            "30": +0.014,  # 門別 (+0.07秒/km)
            "35": -0.398,  # 盛岡 (-1.99秒/km, NAR最速)
            "36": -0.157,  # 水沢 (-0.79秒/km)
            "42": -0.110,  # 浦和 (-0.55秒/km)
            "43": +0.006,  # 船橋 (ほぼ平均)
            "44": -0.195,  # 大井 (-0.97秒/km)
            "45": +0.101,  # 川崎 (+0.51秒/km)
            "46": -0.077,  # 金沢 (-0.38秒/km)
            "47": +0.108,  # 笠松 (+0.54秒/km)
            "48": +0.054,  # 名古屋 (+0.27秒/km)
            "50": +0.331,  # 園田 (+1.66秒/km)
            "51": +0.417,  # 姫路 (+2.08秒/km, NAR最遅)
            "54": +0.183,  # 高知 (+0.92秒/km)
            "55": +0.042,  # 佐賀 (+0.21秒/km)
        },
    }
