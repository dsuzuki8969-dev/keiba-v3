...
# ============================================================
# 2026-06-21 マスター指示: メイクデビュー・障害は買わない
# ============================================================


def is_no_bet_race_type(race_name: str) -> bool:
    """メイクデビュー（新馬戦）・障害レースは買い目対象外と判定する。

    マスター指示 (2026-06-21): 経験の浅い新馬戦と障害戦は買わない。
    障害戦は surface が '芝' 扱いで保存されるため surface では判別できず、
    レース名に「メイクデビュー」「障害」を含むかで判定する。
    engine.analyze (RaceInfo.race_name) と regen_strategy (race['name']) の
    双方から呼ばれる共通ヘルパー。
    """
    if not race_name:
        return False
    return ("メイクデビュー" in race_name) or ("障害" in race_name)
...
# D-3 ROLLBACK (2026-05-25 緊急): マスター指示「フォーメーションは勝手に変えるな」
# → A を C パターンに戻す (5/24 以前の元仕様)
_M_PRIME_STRATEGY: Dict[str, Optional[str]] = {
    "SS": "E",
    "S":  "C",
    "A":  "C",   # D-3 ROLLBACK: D→C 復元
    "B":  "D",
    "C":  "D",
    "D":  "D",
    "E":  None,   # skip
